...
# regular season but, being one cell per matchup, drops the repeat games of a
# best-of-N play-off series; the team PROGRAM pages list every game individually
# and recover them. Team ids are read live from the nav bar, so nothing is hardcoded.
phase_seasons = c("00000001", "00000002")
...
# pull every unique value of `key` (e.g. "idgame", "idteam") from a page's links
pull_ids = function(page, key) {
  hrefs = page %>% html_elements("a") %>% html_attr("href")
  v = extract_id(hrefs, key)
  unique(v[!is.na(v)])
}
...
team_url = function(idteam, mode = 3)   # mode=3 = PROGRAM (full schedule)
  paste0(BASE_URL, "/en/action/EsaketeamView?idteam=", idteam, "&mode=", mode)

# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#' *EXTRACT MATCH IDs*
# Union of: (a) each phase crosstab (mode=2) and (b) each team's PROGRAM page
# (mode=3). (a) is complete for the regular season; (b) recovers the best-of-N
# play-off games the crosstab collapses. Dedup at the end.
esake_game_ids = function() {
  ids = character(0)
  team_ids = character(0)

  for (s in phase_seasons) {
    page = tryCatch(fetch_html(results_url(s, "01", 2)), error = function(e) NULL)
    if (is.null(page)) next
    ids = c(ids, pull_ids(page, "idgame"))
    team_ids = c(team_ids, pull_ids(page, "idteam"))   # nav bar -> all teams
  }
  team_ids = unique(team_ids)

  for (tid in team_ids) {
    page = tryCatch(fetch_html(team_url(tid, 3)), error = function(e) NULL)
    if (!is.null(page)) ids = c(ids, pull_ids(page, "idgame"))
  }
  unique(ids)
}
